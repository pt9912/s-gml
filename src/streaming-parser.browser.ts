import { GmlParser } from './parser.browser.js';
...
export class StreamingGmlParser {
    ...
    async parseFile(filePath: string): Promise<void> {
        throw new Error(`parseFile is not available in the browser build of @npm9912/s-gml: ${filePath}`);
    }
...
export async function parseGmlStream(
    ...
    if (typeof stream === 'string') {
        if (stream.startsWith('http://') || stream.startsWith('https://')) {
            ...
            throw new Error('parseGmlStream only accepts URLs or Web Streams in the browser build of @npm9912/s-gml.');
        }
    } else {
